      ...
      <div class="row mb-4">
        ...
        @if((Auth::user()->role == "Fournisseur") || (Auth::user()->privilege == "admin") ) 
          <div class="col-lg-10 justify-content-between">
            <a href="/" class="btn btn-primary mb-4 max_width">Liste des produits et services</a>
          </div>
        @endif
              ...
              <div class="card col-lg-5 col-md-4 mx-2 my-2 border_radius">
                <div>
                  <form action="/members/{{$li->id}}/state" method="POST">
                    ...
                    @elseif($li->active === 'Yes')
                    <button type="submit" class="btn btn-outline-danger btn-sm mt-1 mx-3">Désactiver</button>
                    @endif
                    @if(!(Auth::user()->privilege == "admin"))
                    <button type="submit" class="btn btn-outline-warning btn-sm mt-1 mr-3">Donner les droits administrateur</button>
                    @elseif(Auth::user()->privilege == "admin")
                    <button type="submit" class="btn btn-outline-success btn-sm mt-1 mr-3">Retirer les droits administrateur</button>
                    @endif
                  </form>
